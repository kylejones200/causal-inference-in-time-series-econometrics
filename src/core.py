...
def perform_granger_causality_test(data: pd.DataFrame, var1: str, var2: str, maxlag: int=4) -> dict:
    """Perform Granger causality test."""
    test_data = data[[var1, var2]]
    result = grangercausalitytests(test_data, maxlag=maxlag, verbose=False)
    return result

def difference_in_differences(y_treated: np.ndarray, y_control: np.ndarray, treatment_period: int) -> float:
