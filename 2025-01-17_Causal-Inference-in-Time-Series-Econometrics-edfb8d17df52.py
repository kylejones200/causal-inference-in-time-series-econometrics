...
class GrangerAnalysis:
    ...
    def test_granger_causality(self, variable1, variable2, max_lags=12):
        ...
        causality_results = pd.DataFrame(
            index=range(1, max_lags + 1), columns=["F-statistic", "p-value"]
        )
...
class LocalProjections:
    ...
    def estimate_impulse_response(
        self, dependent_var, shock_var, controls=None, horizons=20
    ):
...
class SyntheticControl:
    def __init__(
        self, data, treatment_unit, control_units, treatment_period, outcome_var
    ):
        self.data = data
...
class DynamicDiD:
    ...
    def estimate_dynamic_effects(
        self, outcome_var, treatment_var, unit_fe=True, time_fe=True
    ):
...
class ModernCausalInference:
    ...
    def double_machine_learning(self, y, d, x, ml_model=None):
        ...
        treatment_effect = sm.OLS(self.data[y] - y_hat, self.data[d] - d_hat).fit()
